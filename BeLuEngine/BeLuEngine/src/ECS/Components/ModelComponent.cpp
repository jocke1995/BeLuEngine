...
namespace component
{
	...
	void ModelComponent::SetModel(Model* model)
	{
		...
		m_SlotInfoByteAdressBuffer = IGraphicsBuffer::Create(E_GRAPHICSBUFFER_TYPE::RawBuffer, E_GRAPHICSBUFFER_UPLOADFREQUENCY::Static, sizeof(SlotInfo)	 , numMeshes, DXGI_FORMAT_UNKNOWN, to_wstring(fileName) + L"_RAWBUFFER_SLOTINFO");
		m_MaterialByteAdressBuffer = IGraphicsBuffer::Create(E_GRAPHICSBUFFER_TYPE::RawBuffer, E_GRAPHICSBUFFER_UPLOADFREQUENCY::Static, sizeof(MaterialData), numMeshes, DXGI_FORMAT_UNKNOWN, to_wstring(fileName) + L"_RAWBUFFER_MATERIALDATA");
	}
